  ...
  <div class="container">
    <div class="page-inner">
      <div class="page-header">
        <h3 class="fw-bold mb-3">{{ route.meta.title || 'Thêm Thương Hiệu Mới' }}</h3>
        ...
        <div class="card-body">
          <form @submit.prevent="addBrand">
            ...
            <div class="mb-3">
              ...
              <input type="file" class="form-control" id="brandLogo" @change="handleLogoUpload">
              <small class="form-text text-muted">Chọn file ảnh logo (JPG, PNG, GIF, WebP).</small>
              <div v-if="errors.logo" class="text-danger">{{ errors.logo[0] }}</div>
              <div v-if="brand.logoPreview" class="mt-2">
                <img :src="brand.logoPreview" alt="Logo Preview" style="width: 100px; height: 100px; object-fit: contain;">
              </div>
            </div>
            <div class="mb-3">
...
const brand = reactive({
  ...
  logoPreview: '', // For displaying image preview
  description: ''
});
...
const addBrand = async () => {
  ...
  try {
    const response = await axios.post("http://localhost:8000/api/admin/brands", formData, {
      headers: {
    ...
    Swal.fire({
      ...
      router.push({ name: 'BrandList' }); // Chuyển về trang danh sách sau khi thêm thành công
    });
    ...
    if (error.response && error.response.data && error.response.data.errors) {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Lỗi!',
        text: error.response?.data?.message || 'Có lỗi xảy ra khi thêm thương hiệu.',
        confirmButtonText: 'Đã hiểu!'
      });
    }
  } finally {
