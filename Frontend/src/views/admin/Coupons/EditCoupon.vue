...
export default {
  ...
  methods: {
    async fetchCoupon() {
      try {
        const response = await axios.get(`http://localhost:8000/api/coupons/${this.$route.params.id}`);
        this.coupon = {
    ...
    async updateCoupon() {
      ...
      try {
        const response = await axios.put(`http://localhost:8000/api/coupons/${this.$route.params.id}`, {
