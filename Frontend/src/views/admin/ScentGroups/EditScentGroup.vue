...
export default {
  ...
  methods: {
    async fetchScentGroup() {
      try {
        const response = await axios.get(`http://localhost:8000/api/admin/scent-groups/${this.$route.params.id}`);
        this.scentGroup = {
    ...
    async updateScentGroup() {
      ...
      try {
        const response = await axios.put(`http://localhost:8000/api/admin/scent-groups/${this.$route.params.id}`, {
