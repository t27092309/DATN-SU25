            ...
            >
              <div class="mb-6">
                <h5 class="text-xl font-semibold text-gray-800">
                  {{
                    isEditing ? "Chỉnh sửa nhóm hương" : "Thêm mới nhóm hương"
                  }}
                </h5>
              </div>
              <div class="mb-4">
...
const fetchScentGroups = async () => {
  try {
    ...
    scentGroups.value = Array.isArray(response.data)
      ? response.data
      : response.data.data || [];
...
const toggleScentGroup = async (id) => {
  try {
    const response = await axios.put(
      `http://localhost:8000/api/admin/scent-groups/${id}/toggle`
    );
    toast.success(response.data.message || "Cập nhật trạng thái thành công!");
    await fetchScentGroups();
  } catch (error) {
    console.error("Lỗi khi toggle nhóm hương:", error);
    toast.error(
  ...
  try {
    ...
    if (result.isConfirmed) {
      const response = await axios.delete(
        `http://localhost:8000/api/admin/scent-groups/${id}`
      );
      await fetchScentGroups();
    ...
    const errorMessage =
      ...
      "Không kết nối được tới server. Vui lòng kiểm tra mạng của bạn.";
...
const destroyAndReinitializeDataTable = async () => {
  ...
  if (scentGroups.value.length > 0) {
    ...
    if (typeof jQuery !== "undefined" && jQuery.fn.DataTable) {
      ...
      console.error(
        "DataTables không được tải đúng cách hoặc không có jQuery."
      );
    }
  }
};
