                ...
                <div class="p-6">
                    <form @submit.prevent="updateProduct">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-6">
                            <div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <p v-if="errors.gender" class="text-red-500 text-xs italic mt-1">{{ errors.gender }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div v-if="currentImageUrl" class="mt-2 flex items-center space-x-4">
                                    <label class="block text-gray-700 text-sm font-bold">Ảnh hiện tại:</label>
                                    <div class="relative group">
                                        <img :src="currentImageUrl" alt="Ảnh chính"
                                            class="w-32 h-32 object-cover rounded-md shadow-md border border-gray-200" />
                                        <button type="button" @click="removeMainImage = true"
                        ...
                        <div class="mb-6">
                            ...
                            <p v-if="errors.description" class="text-red-500 text-xs italic mt-1">{{ errors.description
                            }}</p>
                        </div>
                        ...
                        <div class="mb-8">
                            <h4 class="text-xl font-semibold mb-4 text-gray-800">Thư viện hình ảnh</h4>
                            <label for="galleryImages" class="block text-gray-700 text-sm font-bold mb-2">Chọn thêm
                                ảnh</label>
                            <input type="file" multiple
                                class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 file:cursor-pointer"
                                @change="onGalleryFilesChange" accept="image/*" />
                            <div v-if="product.gallery_images.length > 0"
                                class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                                <div v-for="(image, index) in product.gallery_images" :key="image.id"
                                    class="relative group">
                                    <img :src="getImageUrl(image.path)" :alt="'Gallery Image ' + (index + 1)"
                                        class="w-full h-32 object-cover rounded-md shadow-sm border border-gray-200" />
                                    <button type="button" @click="removeGalleryImage(image)"
                                        class="absolute top-1 right-1 bg-red-600 text-white rounded-full p-1 text-xs opacity-0 group-hover:opacity-100 transition-opacity">
                                        <i class="fas fa-times"></i>
                                    </button>
                                </div>
                        ...
                        <ScentGroupSelector :selected-scent-group-ids="product.scentGroups.selectedScentGroupIds"
                            @update:selected-scent-group-ids="newIds => product.scentGroups.selectedScentGroupIds = newIds"
                            :scent-groups-data="product.scentGroups.scentGroupsData"
                            @update:scent-groups-data="newData => product.scentGroups.scentGroupsData = newData"
                        ...
                        <div v-if="product.has_variants === true">
                            ...
                            <div>
                                <p v-if="errors.variants" class="text-red-500 text-xs italic mb-4">{{ errors.variants[0]
                                    }}</p>
                                    ...
                                    <div class="flex flex-wrap gap-x-6 gap-y-3">
                                        <div v-for="value in attribute.attribute_values" :key="value.id"
                                            ...
                                            <input
                                                ...
                                                :value="value.id" v-model="selectedAttributeValues[attribute.id]" />
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="(variant, index) in product.variants" :key="index"
                                            ...
                                            <td class="py-3 px-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                {{variant.attributes.map(a => a.value_name).join(' - ')}}
                                            </td>
                                            <td class="py-3 px-4 whitespace-nowrap text-sm text-gray-700">
                                                <input type="text"
                                                    ...
                                                    v-model="variant.sku" :id="'variantSku' + index" />
                                            ...
                                            <td class="py-3 px-4 whitespace-nowrap text-sm text-gray-700">
                                                <input type="number"
                                                    class="w-full text-sm py-1 px-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
                                                    v-model="variant.price" min="0" :id="'variantPrice' + index" />
                                            ...
                                            <td class="py-3 px-4 whitespace-nowrap text-sm text-gray-700">
                                                <input type="number"
                                                    class="w-full text-sm py-1 px-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
                                                    v-model="variant.stock" min="0" :id="'variantStock' + index" />
                            ...
                            <p v-else
                                ...
                                thuộc tính và giá trị ở trên để tạo biến thể.
                            </p>
                        </div>
...
const populateSelectedAttributeValues = (variants) => {
    ...
    const allAttributeValueIds = variants.flatMap(v => v.attributes.map(attr => attr.value_id));
...
const fetchAttributes = async () => {
    try {
        const response = await axios.get('http://localhost:8000/api/admin/attributes');
...
const fetchProduct = async () => {
    try {
        const { data } = await axios.get(`http://localhost:8000/api/admin/products/${productId.value}`);
        ...
        if (fetchedProduct.scent_profiles) {
            product.value.scentGroups.selectedScentGroupIds = fetchedProduct.scent_profiles.map(sp => sp.scent_group_id);
            product.value.scentGroups.scentGroupsData = fetchedProduct.scent_profiles.reduce((acc, sp) => {
        ...
        if (fetchedProduct.images && Array.isArray(fetchedProduct.images)) {
            product.value.gallery_images = fetchedProduct.images.map((url, index) => ({
        ...
        if (product.value.has_variants && fetchedProduct.variants && fetchedProduct.variants.length > 0) {
...
const generateVariants = () => {
    ...
    const activeAttributeValueIdGroups = Object.values(selectedAttributeValues.value)
    ...
    const combinations = activeAttributeValueIdGroups.reduce((acc, currentGroup) => {
...
const removeSpecificVariant = (index) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const attributeValuesToDelete = variantToDelete.attributes.map(attr => attr.value_id);

            product.value.variants.splice(index, 1);

            const remainingValues = product.value.variants.flatMap(v => v.attributes.map(a => a.value_id));

            for (const attrId in selectedAttributeValues.value) {
                selectedAttributeValues.value[attrId] = selectedAttributeValues.value[attrId].filter(val =>
                    remainingValues.includes(val.valueId)
                );
            }
    ...
    try {
        ...
        for (const key in product.value) {
            if (key === 'has_variants') {
                ...
                if (formattedScentGroups.length > 0) {
                    formData.append('scent_groups', JSON.stringify(formattedScentGroups));
                }
            } else if (key === 'usageProfile') {
                for (const upKey in product.value.usageProfile) {
                    const value = product.value.usageProfile[upKey];
                    if (value !== null && value !== undefined) {
                        formData.append(`usage_profile[${upKey}]`, value);
                    }
                }
            } else if (key !== 'slug' && key !== 'variants' && key !== 'gallery_images' && product.value[key] !== null && product.value[key] !== '') {
        ...
        // Nếu người dùng chọn xóa ảnh chính
        ...
        if (product.value.has_variants) {
            const variantsData = product.value.variants.map((variant, index) => {
                const variantCopy = { ...variant };
                variantCopy.attribute_value_ids = variantCopy.attributes.map(attr => attr.value_id) || [];
                delete variantCopy.attributes;
                delete variantCopy.imageFile;
                delete variantCopy.imageUrlPreview;
                return variantCopy;
            });
        ...
        await axios.post(`http://localhost:8000/api/admin/products/${productId.value}`, formData, {
            // headers: {
...
const sortedScentProfiles = computed(() => {
    ...
    const profiles = selectedIds.map(id => {
...
onMounted(async () => {
    ...
    unwatchVariants = watch(selectedAttributeValues, updateVariants, { deep: true });
