            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr
                    class="text-center"
                    v-for="(product, index) in trashedProducts"
                    :key="product.id"
                  >
                    <td
                    ...
                    <td
                      class="px-6 py-4 whitespace-nowrap text-sm font-medium flex flex-col space-y-2 items-center justify-center"
                    >
                      <button
                        @click="confirmRestoreProduct(product.id)"
                      ...
                      <button
                        @click="confirmForceDeleteProduct(product.id)"
...
const fetchTrashedProducts = async () => {
  try {
    ...
    const response = await axios.get(
      "http://localhost:8000/api/admin/products/trashed"
    );
...
const confirmRestoreProduct = async (id) => {
  ...
  if (result.isConfirmed) {
    try {
      await axios.post(
        `http://localhost:8000/api/admin/products/${id}/restore`
      );
      ...
      Swal.fire({
        icon: "error",
        title: "Lỗi!",
        text: `Không thể khôi phục sản phẩm: ${
...
const confirmForceDeleteProduct = async (id) => {
  ...
  if (result.isConfirmed) {
    try {
      await axios.delete(
        `http://localhost:8000/api/admin/products/${id}/force-delete`
      );
      await fetchTrashedProducts(); // Tải lại danh sách thùng rác
      Swal.fire({
      ...
      Swal.fire({
        icon: "error",
        title: "Lỗi!",
        text: `Không thể xóa vĩnh viễn sản phẩm: ${
          error.response?.data?.message || error.message
        }`,
      });
    }
