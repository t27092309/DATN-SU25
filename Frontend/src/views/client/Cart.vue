                    ...
                    <div v-for="item in cartData.items" :key="item.id"
                        ...
                        <div class="flex items-center space-x-3 col-span-1">
                            ...
                            <div class="flex-1">
                                ...
                                <div v-if="item.variant" class="flex items-center text-gray-500 text-sm mt-1">
                                    <span>Phân Loại Hàng:</span>
                                    <div class="relative flex-grow min-w-0"> <select
                                            class="border rounded-md ml-2 px-2 py-1 text-sm appearance-none block w-full truncate pr-8 cursor-not-allowed"
                                            :value="item.variant.id" disabled
                                            @change="event => handleChangeVariant(item.id, event.target.value)">
                                            <option :value="item.variant.id" class="truncate">
                                                {{ truncateText(item.variant.name, 30) }}
                                                <span
                                                    v-if="item.variant.attributes && item.variant.attributes.length > 0">
                                                    ({{ truncateText(item.variant.attributes.map(attr =>
                                                    `${attr.attribute_name}: ${attr.value}`).join(', '), 40) }})
                                                </span>
                                            </option>
                                        </select>
                                        <div
...
const isAllSelected = computed(() => {
    ...
    return selectedItems.value.size === cartData.value.items.length;
});
...
const fetchCartData = async () => {
    ...
    try {
        ...
        // Tự động chọn tất cả sản phẩm sau khi tải nếu giỏ hàng không rỗng
...
const handleChangeQuantity = async (cartItemId, newQuantity) => {
    ...
    if (itemIndex !== -1) {
        ...
        try {
            ...
            // Sau khi thành công, có thể fetch lại toàn bộ giỏ hàng
            // hoặc API trả về dữ liệu cập nhật và bạn chỉ cập nhật item đó.
            await fetchCartData(); // Tải lại toàn bộ dữ liệu để đảm bảo đồng bộ hoàn toàn
        } catch (err) {
            console.error('Lỗi khi cập nhật số lượng:', err);
            error.value = 'Không thể cập nhật số lượng. Vui lòng thử lại.';
 ...
 * (Hiện tại chỉ là placeholder, cần logic để lấy tất cả biến thể khả dụng)
 * @param {number} cartItemId - ID của cart item.
...
const handleChangeVariant = async (cartItemId, newVariantId) => {
    ...
    try {
        ...
        await fetchCartData(); // Tải lại toàn bộ dữ liệu để đảm bảo đồng bộ
    } catch (err) {
...
const handleRemoveSelectedItems = async () => {
    ...
    try {
        // Tạm thời tạo một mảng ID để gửi lên server
        const itemIdsToDelete = Array.from(selectedItems.value);
        // Đây là một cách tiếp cận: gửi nhiều yêu cầu DELETE
        // Hoặc bạn có thể tạo một API endpoint mới trên backend để xóa nhiều item cùng lúc.
