...
const getPriceDisplay = (product) => {
  if (product.variants && product.variants.length > 0) {
    ...
    if (minPrice === maxPrice) {
      return `${formattedMin} VNĐ`;
    }
    return `${formattedMin} - ${formattedMax} VNĐ`;
  } else if (product.price) {
    const formattedPrice = new Intl.NumberFormat('vi-VN').format(parseFloat(product.price));
...
const filteredProducts = computed(() => {
  ...
  if (selectedPriceRange.value) {
    ...
    result = result.filter(product => {
      // Logic lọc dựa trên giá thấp nhất của variant
      if (product.variants && product.variants.length > 0) {
        const prices = product.variants.map(variant => parseFloat(variant.price));
        const minProductPrice = Math.min(...prices);
        return !isNaN(minProductPrice) && (min === null || minProductPrice >= min) && (max === null || minProductPrice <= max);
      }
      return false; // Sản phẩm không có variants sẽ không được lọc bằng giá
    });
  }
  ...
  if (selectedAromas.value.length > 0) {
    result = result.filter(product => {
      ...
      const productAromas = product.scent_profiles.map(
        (profile) => profile.scent_group.name
      );
