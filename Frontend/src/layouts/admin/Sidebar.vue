      ...
      <ul class="space-y-1">
        <li v-for="menuItem in menuItems" :key="menuItem.name" class="relative">
          ...
          <router-link 
            ...
            <!-- Icon Container -->
            <div class="relative">
              <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center group-hover:bg-gradient-to-br group-hover:from-blue-500 group-hover:to-purple-600 group-[.router-link-exact-active]:bg-gradient-to-br group-[.router-link-exact-active]:from-blue-500 group-[.router-link-exact-active]:to-purple-600 transition-all duration-300 shadow-sm">
                <i v-if="menuItem.icon" :class="[menuItem.icon, 'text-gray-600 group-hover:text-white group-[.router-link-exact-active]:text-white transition-colors duration-300']"></i>
              </div>
              <!-- Active indicator dot -->
              <div class="absolute -top-1 -right-1 w-3 h-3 bg-gradient-to-r from-blue-400 to-purple-500 rounded-full opacity-0 group-[.router-link-exact-active]:opacity-100 transition-opacity duration-300 shadow-md"></div>
            </div>
            
            <span class="ml-4 font-medium text-gray-700 group-hover:text-gray-900 group-[.router-link-exact-active]:text-blue-700 transition-colors duration-300">
              {{ menuItem.name }}
            </span>
          ...
          <div v-else class="relative">
            <div
              ...
              :class="{
                'bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 shadow-md text-blue-700': menuItem.isOpen,
                'hover:bg-gray-100 text-gray-700 hover:text-gray-900': !menuItem.isOpen
              }">
              
              <div class="flex items-center">
                <!-- Icon Container -->
                <div class="relative">
                  <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center transition-all duration-300 shadow-sm"
                       :class="{
                         'bg-gradient-to-br from-blue-500 to-purple-600': menuItem.isOpen,
                    ...
                    <i v-if="menuItem.icon" 
                       :class="[
                         ...
                         menuItem.isOpen ? 'text-white' : 'text-gray-600 group-hover:text-gray-800'
                       ]"></i>
                  </div>
                  <!-- Active indicator dot -->
                  <div class="absolute -top-1 -right-1 w-3 h-3 bg-gradient-to-r from-blue-400 to-purple-500 rounded-full transition-opacity duration-300 shadow-md"
                       :class="{ 'opacity-100': menuItem.isOpen, 'opacity-0': !menuItem.isOpen }"></div>
                </div>
              ...
              <!-- Dropdown Arrow -->
              <div class="relative">
                <svg class="w-5 h-5 transition-all duration-300 text-gray-400"
                     :class="{ 
            ...
            <Transition name="accordion" @before-enter="onBeforeEnter" @enter="onEnter" @after-enter="onAfterEnter"
              ...
              <div v-if="menuItem.children && menuItem.isOpen" class="mt-2 mx-2 mb-2">
                  ...
                  <ul class="py-2">
                    <li v-for="subItem in menuItem.children" :key="subItem.name">
                      <router-link :to="subItem.route"
                        ...
                        <!-- Connecting line -->
                        <div class="w-6 h-px bg-gradient-to-r from-gray-300 to-transparent mr-2"></div>
                        
                        <!-- Submenu indicator dot -->
                        <div class="w-2 h-2 rounded-full bg-gray-400 group-hover:bg-blue-500 group-[.router-link-exact-active]:bg-blue-500 transition-colors duration-200 mr-3 flex-shrink-0"></div>
                        
                        <span class="text-sm text-gray-600 group-hover:text-gray-900 group-[.router-link-exact-active]:text-blue-600 transition-colors duration-200 font-medium">
                          {{ subItem.name }}
                        </span>
                        
                        <!-- Hover background effect -->
                        <div class="absolute inset-0 bg-gradient-to-r from-blue-600/0 to-purple-600/0 group-hover:from-blue-600/5 group-hover:to-purple-600/5 transition-all duration-200"></div>
                      </router-link>
                    </li>
                  </ul>
                </div>
